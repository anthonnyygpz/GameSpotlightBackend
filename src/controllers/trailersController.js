 ...
 */
exports.getByGameId = async (req, res, next) => {
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Trailer no encontrado para este juego' });
    }
...
exports.getById = async (req, res, next) => {
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Trailer no encontrado' });
    }
