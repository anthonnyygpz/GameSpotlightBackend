 ...
 */
const buildGameItem = (row: GameRow) => ({
...
export const getAllGames = async (
  ...
  try {
    ...
    const [rows] = await db.execute<(GameRow & RowDataPacket)[]>(
    ...
    const [countRows] = await db.execute<({ total: number } & RowDataPacket)[]>(
 ...
 * Secciones: hero, trailers, upcoming, top_rated, new_releases, exclusives
 */
export const getHomeGames = async (
  ...
  try {
    ...
    const [heroRows] = await db.execute<(GameRow & RowDataPacket)[]>(
      `SELECT vg.*
    ...
    const [trailerRows] = await db.execute<(GameRow & RowDataPacket)[]>(
    ...
    const [upcomingRows] = await db.execute<(GameRow & RowDataPacket)[]>(
    ...
    const [topRatedRows] = await db.execute<(GameRow & RowDataPacket)[]>(
      `SELECT vg.*
       FROM v_games_full vg
    ...
    const [newReleasesRows] = await db.execute<(GameRow & RowDataPacket)[]>(
      `SELECT vg.*
       FROM v_games_full vg
    ...
    // 6) Exclusives
    const [exclusivesRows] = await db.execute<(GameRow & RowDataPacket)[]>(
      `SELECT DISTINCT vg.*
       FROM v_games_full vg
...
export const searchGames = async (
  ...
  try {
    ...
    if (!(q as string).trim()) {
      res.status(422).json({ success: false, message: 'El parámetro q es requerido' });
      return;
    }
...
export const getGameById = async (
  ...
  try {
    ...
    const [gameRows] = await db.execute<(GameRow & RowDataPacket)[]>(
    ...
    interface TrailerDetailRow extends RowDataPacket {
      trailer_id: string;
      game_id: string;
      title: string;
      type: string;
    ...
    const [trailerRows] = await db.execute<TrailerDetailRow[]>(
